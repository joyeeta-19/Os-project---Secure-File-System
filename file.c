...
/*
 * Map the buffer_head passed in argument with the iblock-th block of the file represented by inode. If the requested block is not allocated and create is true,  allocate a new block on disk and map it.
 */
static int myfs_file_get_block(struct inode *inode,sector_t iblock,struct buffer_head *bh_result,int create)
{
    struct super_block *sb = inode->i_sb;	//super block structure
    struct myfs_sb_info *sbi = MYFS_SB(sb);//super block info
    struct myfs_inode_info *ci = MYFS_INODE(inode);	//inode info
    struct myfs_file_ei_block *index;
    struct buffer_head *bh_index;
    bool alloc = false;	//allocation is initially false
    int ret = 0, bno;
    uint32_t extent;	//!!!!!!!
    ...
    if (index->extents[extent].ee_start == 0) {
        ...
        index->extents[extent].ee_block =
            extent ? index->extents[extent - 1].ee_block +index->extents[extent - 1].ee_len
                   : 0;
        alloc = true;
    } else {
...
/*
 * Called by the VFS when a write() syscall occurs on file before writing the data in the page cache. This functions checks if the write will be able to complete and allocates the necessary blocks through block_write_begin().
 */
...
/*
 * Called by the VFS after writing data from a write() syscall to the page cache. This functions updates inode metadata and truncates the file if necessary.
 */
